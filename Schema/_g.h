#pragma once

#include <assert.h>
#include "include\utility.h"
#include "include\itable.h"

static int __tolower_table[0x100] = {
	
};

#ifndef char_tolower
#define char_tolower(ch)	(__tolower_table[(ch) & 0xff])
#endif

inline void s2l(char* dst, const char* src, unsigned len)
{
	register int mb = 0;
	register char ch = 0;
	for (unsigned i = 0; i < len; ++i)
	{
		ch = src[i];
		dst[i] = mb ? ch : char_tolower(ch);
		if (mb) mb = 0;
		else mb = ch & 0x80;
	}
	dst[len] = 0;
}

static int __xxx_table[0x100] = {
	
};

#ifndef char_blank
#define char_blank(ch)		(__xxx_table[(unsigned char)(ch)] & 0x01)
#endif

#ifndef char_newline
#define char_newline(ch)	(__xxx_table[(unsigned char)(ch)] & 0x02)
#endif

#ifndef char_space
#define char_space(ch)		(__xxx_table[(unsigned char)(ch)] & 0x04)
#endif

#ifndef NELEMS
#define NELEMS(x)		(sizeof((x))/(sizeof(((x)[0]))))
#endif

unsigned normalize_path(char* out, unsigned count, const char* in);

struct BUFF { void* addr; unsigned size; };

namespace Schema
{
	hfile OpenReadFile(const char* pcszFile);
	hfile OpenWriteFile(const char* pcszFile);
	void CloseFile(hfile hFile);
	unsigned TellFileLength(hfile hFile);
	unsigned FileRead(hfile hFile, void* pvBuff, unsigned uReadBytes);
	unsigned FileWrite(hfile hFile, void* pvBuff, unsigned uWriteBytes);
}
